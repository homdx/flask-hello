...
ARG DOT_VERSION=0.0.6
ARG DOT_PATH=https://github.com/homdx/flask-hello/releases/download
ARG DOT_FILE1=local-bin2.tar.gz
ARG DOT_HASH1=c6b45d35df0157539f5462b4e37642e663ff0ceb0465a933411bd59078ffd3e3
ARG DOT_FILE2=root-gradle2.tar.gz
ARG DOT_HASH2=89b5aa2997c56fdf54212f14695881b0143db365817696f412472255aec88f05
#Fix https://github.com/kivy/python-for-android/issues/1749

ARG DISABLECACHE=${DISABLE_CACHE}

RUN apt-get install aria2 time --yes
...
RUN set -ex && \
    ...
    set -ex ; \
    cd / ; time -p aria2c -x 5 ${DOT_PATH}/${DOT_VERSION}/${DOT_FILE1} ; \
    echo "${DOT_HASH1}  ${DOT_FILE1}" | sha256sum -c ; \
    time -p tar -xf ${DOT_FILE1} ; rm ${DOT_FILE1} ; \
    cd /root ; time -p aria2c -x 5 ${DOT_PATH}/${DOT_VERSION}/${DOT_FILE2} ; \
...
ADD main.py flask-hello/main.py
ADD buildcmd.sh flask-hello/buildcmd.sh

WORKDIR /home/userhome/flask-hello

RUN /home/userhome/flask-hello/buildcmd.sh

RUN echo tail -f testfile >>/home/userhome/flask-hello/buildcmd.sh

CMD /home/userhome/flask-hello/buildcmd.sh
#CMD tail -f /home/userhome/flask-hello/main.py
